	...
	if(request.method == 'POST'):
		if form.is_valid():
			...
			if not roomModel.objects.filter(roomName=roomName).exists():
				...
				if roomType == 'private':
					return redirect('PerformRSA:putPassword', room_name=roomName)

				messages.success(request, 'Room successfully created')
				return redirect('PerformRSA:room', room_name=roomName)
			else:
				messages.info(request, 'Room already exists')
				return redirect('PerformRSA:createRoom')
		else:
...
def putPassword(request, room_name):
	...
	if request.method == 'POST':
		if form.is_valid():
			...
			room.save()
			return redirect('PerformRSA:room', room_name=room_name)
	else:
		form = passwordForm()
	context = {
		'room' : room,
